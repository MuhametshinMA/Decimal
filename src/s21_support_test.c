...
void set_origin_decs(Origin_dec* dec, unsigned char index) {
  ...
  snprintf(format, sizeof(format), "mod_%u_dec_%u.1", index, index);
  set_dec_from_file(&dec->mod1, format);
  memset(format, '\0', sizeof(format));

  snprintf(format, sizeof(format), "mod_%u_dec_%u.2", index, index);
  set_dec_from_file(&dec->mod2, format);
  memset(format, '\0', sizeof(format));
