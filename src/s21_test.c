...
START_TEST(mod_test) {
  for (unsigned char i = 0; i < count_line; i++) {
    ...
    s21_decimal dec1 = origin_dec.dec1;
    s21_decimal dec2 = origin_dec.dec2;
    s21_decimal dec_mod1 = {0};
    s21_mod(dec1, dec2, &dec_mod1);
    char* s21_mod1 = print_dec(dec_mod1);
    char* ori_mod1 = print_dec(origin_dec.mod1);
    ck_assert_str_eq(s21_mod1, ori_mod1);
    free(s21_mod1);
    free(ori_mod1);

    s21_decimal dec_mod2 = {0};
    s21_mod(dec2, dec1, &dec_mod2);
    char* s21_mod2 = print_dec(dec_mod2);
    char* ori_mod2 = print_dec(origin_dec.mod2);
    ck_assert_str_eq(s21_mod2, ori_mod2);
    free(s21_mod2);
    free(ori_mod2);
  }
}
...
Suite* arithmetic_suite(void) {
  ...
  tcase_add_test(tc_arithmetic, mod_test);
