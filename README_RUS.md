...
В этом проекте реализована библиотека s21_decimal.h на языке программирования Си. Эта библиотека позволяет работать с типом "decimal", который отсутствует в стандарте языка.
...
- *Если в результате операции произошло переполнение, отбрасывается дробная часть (например, 70,000,000,000,000,000,000,000,000,000 % 0.001 = 0.000)*
...
- *Если числа слишком малы (0 < |x| < 1e-28), функция возвращает ошибку и значение, равное 0*
- *Если числа слишком велики (|x| > 79,228,162,514,264,337,593,543,950,335) или равны бесконечности, функция возвращает ошибку*
